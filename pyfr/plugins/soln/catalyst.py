...
import ctypes
from ctypes import RTLD_GLOBAL, c_char_p, c_int, c_int64, c_void_p
import os
...
def _load_pydll(name, mode):
    # Mirrors pyfr.ctypesutil.load_library but uses PyDLL to keep the GIL
    # held across the catalyst→ParaView→Python C API call chain.
    lpath = os.environ.get(f'PYFR_{name.upper()}_LIBRARY_PATH')
    if lpath:
        return ctypes.PyDLL(lpath, mode=mode)
    lname = platform_libname(name)
    for sd in platform_libdirs():
        try:
            return ctypes.PyDLL(Path(sd, lname).absolute(), mode=mode)
        except OSError:
            pass
    return ctypes.PyDLL(lname, mode=mode)


# Strided external pointer variants — used by CatalystConduitNode to publish
# AoS coords/fields as interleaved zero-copy references (VTK's GetVoidPointer
# fast path).  Only the catalyst pipeline needs these; the base ConduitWrappers
# stays as in pre-PR ascent.
_catalyst_external_functions = [
...
class CatalystConduitWrappers(ConduitWrappers):
    ...
    _mode = RTLD_GLOBAL
    _functions = (
        [(ret, f'catalyst_{fn}', *args)
         for ret, fn, *args in ConduitWrappers._functions] +
        [(ret, f'catalyst_{fn}', *args)
         for ret, fn, *args in _catalyst_external_functions]
    )

    def _load_library(self):
        return _load_pydll(self._libname, self._mode)
...
class CatalystConduitNode(ConduitNode):
    def __setitem__(self, key, value):
        # Strided 1D view → zero-copy external strided pointer (caller keeps
        # the source buffer alive across catalyst_execute via _coord_bufs).
        if isinstance(value, (np.ndarray, np.generic)):
            if value.ndim == 1 and value.strides[0] != value.itemsize:
                fn = getattr(self.lib,
                             f'conduit_node_set_path_external_'
                             f'{value.dtype}_ptr_detailed')
                fn(self, key.encode(), value.ctypes.data, value.size,
    ...
    def append(self):
        ...
        return CatalystConduitNode(self.lib, ptr, child=True)
...
class CatalystRenderer(InSituRenderer):
    def __init__(self, mesh, scfg, cfgsect, isrestart, *, acfg=None):
        ...
        init_n = CatalystConduitNode(self.conduit)
    ...
    def _make_mesh_node(self):
        return CatalystConduitNode(self.conduit)
    ...
    def finalise(self):
        self.lib.catalyst_finalize(CatalystConduitNode(self.conduit))
        self.lib = None
