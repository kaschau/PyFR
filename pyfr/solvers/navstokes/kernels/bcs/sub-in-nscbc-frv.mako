...
% elif ndims == 3:
  ...
  N[0] =  invc*(nx*rho*veloR - c*(nx*rhoR + nz*vR - ny*wR) - ${sq2}*jac*N[4]*nx*rho);
  N[1] =  invc*(ny*rho*veloR - c*(ny*rhoR - nz*uR + nx*wR) - ${sq2}*jac*N[4]*ny*rho);
  N[2] =  invc*(nz*rho*veloR - c*(nz*rhoR + ny*uR - nx*vR) - ${sq2}*jac*N[4]*nz*rho);
